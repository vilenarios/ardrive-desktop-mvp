...
## D-031 · Beta ships with always-prompt login (no auto-login) — UX-6 · CONFIRMED · 2026-07-08 (owner-approved 2026-07-09)
...
**CONFIRMED (owner approved 2026-07-09):** beta ships always-prompt, no auto-login. Implemented as UX-6 (branch `lane/ux6`) in the REMOVE direction: the dead/circular auto-login path (`attemptAutoLoad`, its boot gate, the caller-less `wallet:ensure-loaded` IPC + `preload.ensureLoaded`) is deleted, and the "Remember Me on This Device" Settings control — whose copy promised skipping the password prompt — is withheld for beta (`REMEMBER_ME_ENABLED = false`) because it can no longer keep that promise. SEC-4's opt-in keychain infrastructure remains intact for a post-beta opt-in auto-unlock; if that lands, re-open the "implement" direction under a new item rather than reviving the deleted code verbatim.
...
## D-034 · Proof Mode spec: naming, hybrid integration, phase gates (refines D-028) · PROVISIONAL · 2026-07-09

Fable deep-dive off D-032/GROWTH §7 produced the FEAT-4 spec ([PROOF-MODE-SPEC-2026-07-09.md](./PROOF-MODE-SPEC-2026-07-09.md)), resolving D-028's three open questions. **Naming:** user-facing verb **"Notarize"**, noun **"Proof"**, feature **"Proof Mode"** — "anchor" never appears in the UI (it collides with Arweave's anti-replay field and *sounds like it holds something*; "notarize" imports the right model — a notary does not keep a copy); `anchor` stays in code/internals matching the ar-io-anchor SDK. Two-word state ladder, no synonyms: **Proven** (fingerprint on Arweave, file is not) vs **Preserved** (bytes on Arweave, forever); banned words for proofs (*backed up, stored, saved, protected, secured, safe, archived, uploaded*) enforced by pinned copy tests (DESIGN-8/UX-34 pattern). **Integration (resolves D-028 sub-question b): hybrid** — the ar-io-anchor **envelope** is the proof primitive (one Merkle `batch()` checkpoint per run, ~free, third-party-verifiable via `@ar.io/proof`), and the ArFS presence is **one real Proof Log file *per run*** (gzipped bundle + `ArDrive-Proof-*` tags), **NOT** the naive per-file data-less ArFS entity — which forfeits batch economics and, worse, renders as broken-download files in ardrive-web (an uncontrolled proof≠preservation violation). No filenames on-chain by default (privacy). **Messaging (D-028 risk #1) = the product:** a one-time comprehension gate + a proof-vs-preservation UX that differs on every perceptual channel at once (hollow vs solid badge, 0-B arithmetic, honest "missing-locally" state). **Sequencing:** built behind the beta + the supervised money session; the anchor service builds the streaming SHA-256 utility SYNC-10 then adopts (so it doesn't block on SYNC-10). **Phase gates:** A (proof MVP + free preservation rung) → B (paid funnel, entry-gated on the supervised money session + cost-estimate reconciliation) → C (viral proof pages + daily re-anchor watch mode). One research spike open (§3.4): confirm the ar-io-anchor signer story against our RSA JWK (deps hint ed25519). **Supersedes D-028 when the feature is scheduled** (D-028 stays the "captured post-beta idea" record; D-034 is the how).

## D-035 · Native file-manager overlay badges: build them (GA track) — legacy shell handlers now, Cloud Files/File Provider with FEAT-7 · CONFIRMED · 2026-07-09

**CONFIRMED (Phil, 2026-07-09):** build it; **Windows first**, then macOS; **cert/CI procurement greenlit** (Apple Developer Program + Windows OV Authenticode cert + macOS/Xcode CI runner — Phil to procure; also the GA-signing prerequisite). Phase 0 (shared `OverlayStatusPublisher`) proceeds immediately; native Windows phase begins once the Windows cert + MSVC CI land.

Phil (2026-07-09): the OneDrive/Dropbox per-file overlay badges are a "huge value driver" — "evaluate, plan, implement, thoroughly test." Two grounded platform evaluations produced [OVERLAYS-PLAN-2026-07-09.md](./OVERLAYS-PLAN-2026-07-09.md) (+ research/OVERLAYS-{WINDOWS,MACOS}-EVAL.md). Tracked as **FEAT-9**. Decision on approach: **Windows = legacy Shell Icon Overlay Handler (COM DLL) now**; **macOS = Finder Sync `.appex` embedded via afterPack now**; the modern **Windows Cloud Files API / macOS File Provider is Phase 3, deferred to land with FEAT-7 files-on-demand** (it subsumes overlays and ends the Windows 15-slot war, but forces placeholder/hydration semantics not worth it until we build files-on-demand). Both platforms bridge from our **existing** `sync:file-state-changed` channel through a new platform-agnostic `OverlayStatusPublisher` (fileId→localPath via `drive_metadata_cache`, 7 statuses → 3 badge buckets). **This is GA track — it does NOT block beta** (beta ships on the in-app + tray + notification status surfaces). **Hard dependency (Phil's to procure, and a GA-distribution prerequisite regardless):** a Windows OV Authenticode cert + Apple Developer Program/Developer ID + notarization + a macOS/Xcode CI runner + a manual Win/mac test-machine matrix. Sequencing: Phase 0 (the shared `OverlayStatusPublisher` core) is unblocked and starts now; native phases begin once certs + native CI land, Windows first. Supersedes the "capture, do not build" stance of UX-37.
