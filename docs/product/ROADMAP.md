...
**Track D — GA hardening:** code signing + notarization (Phil-procured cert/CI, per D-035 — the shared prerequisite for the item below and for signed GA distribution), **native file-manager overlay badges (FEAT-9, active — D-035, Windows-first then macOS; [OVERLAYS-PLAN-2026-07-09.md](./OVERLAYS-PLAN-2026-07-09.md))** — the OneDrive/Dropbox per-file green-check/syncing/error cue; Phase 0 (shared `OverlayStatusPublisher` core) is unblocked and proceeds now, native phases begin once the Windows OV Authenticode cert + MSVC/Xcode CI runners land; the modern Cloud Files API / File Provider path is deferred to Phase 3 with FEAT-7 files-on-demand. Plus: secure-logger adoption (SEC-8) + in-app sanitized problem reports (UX-16, D-017), shell confinement (SEC-9), keytar→safeStorage (SEC-10), rate limiting (SEC-11), UX-9/11/12/13/14 polish batch, repo hygiene (INFRA-6/8/11), MONEY-8.
...
**Track F — Crypto-native growth & monetization (D-032; full analysis in [GROWTH-STRATEGY-2026-07-09.md](./GROWTH-STRATEGY-2026-07-09.md)):** beachhead = crypto-native + existing high-volume ArDrive/Arweave users. **Wedge:** "the folder that cannot lie" — the desktop as an ambient permanence daemon, and the D-027 gateway-drop-recovery story ("the desktop found a file the gateway forgot"). **Take-off motion:** folder → named permaweb site (manifest deploy + **ArNS assignment GROWTH-6**), the only feature that emits a public shareable artifact; compounding loops = deploy-badge (GROWTH-3), verifiable permanence-profile flex page (GROWTH-4), sponsored-credits/gift economy (GROWTH-5, leverages FEAT-8 `destinationAddress` + the Console's existing ShareCredits/Gift routes). **Volume caps in kill order:** big-file track (**SYNC-10** streaming hash → 2 GiB cap → resumable uploads), **budget-based consent** (MONEY-16: auto-approve while today's total ≤ N credits + UX-32 free-tier auto-sync — keeps every MONEY-10 invariant, converts approver→daemon), then multi-drive (SYNC-14). **Zero-support triad (elevated priority):** auto-update (INFRA-4 — *the* mechanism by which support cost decays to zero), telemetry (INFRA-5), sanitized log bundle (UX-16). **Monetization:** crypto rail as the *primary* top-up CTA (no chargebacks = zero-support), need-sized top-ups (FEAT-8 `amount=`), real Auto-Top-Up, pay-once-forever receipts. **The growth engine:** **FEAT-4 anchor/proof mode as the FREE funnel** ("notarize everything free" → onboarded+daemon → upsell to preserve) — the top of the funnel that feeds the rest. Plus: market the unmarketed trust story (GROWTH-1 "why you can trust this client" page — 880 adversarial tests, PRIV-8 fail-closed, D-027), the uploader-wallet onboarding pattern (GROWTH-2), and a "Verify my drive" integrity-audit button (GROWTH-7).
