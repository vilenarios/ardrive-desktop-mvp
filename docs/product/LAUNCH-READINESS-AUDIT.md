...
| **Profile switch / create / delete** (`profiles:switch|delete|update`) | SWITCH + ADD FIXED + TESTED 2026-07-06 (branch fix/ux5-profile-switch, UX-5) — both known bugs were renderer-only (main already isolates via SEC-3's `stopAndClearAllState` + `switchProfile`). A successful `profiles.switch` now drives App's `handleProfileSwitched`, which resets ALL profile-scoped state and re-runs `initializeApp` for the new profile (no stale wallet/drives/dashboard; sync-monitoring listeners torn down first). "Add Profile" stops the current sync and routes to onboarding instead of `window.location.reload()` (the old bounce-back loop). Covered by `ux5-app-profile-switch.test.tsx` + `ux5-dashboard-switch-wiring.test.tsx` (new profile's data shown / old gone / wrong-password-no-refresh / no reload). Still not exercised on a real multi-profile machine; `delete`/`update` still untested live. | **P1 — beta feature, switch/add now fixed** |
...
| **Rename/move/copy + edit→revision LIVE execution** | Detection + fileId-reuse proven; on-chain execution env-blocked (SYNC-26 fixed in code, not live). PRIV-6 done (impl) 2026-07-08: private-drive move/rename now routes through core-js's encrypted `*Private*` calls WITH the drive key (public/private/unresolved branch; LOCKED-private fails closed, never a public write; result logged via SEC-1 whitelist so the key never leaks) — unit-tested (+9), but a supervised on-chain private move/rename round-trip is still needed to close it fully | **P1 — needs a real-machine pass (incl. a private move/rename)** |
...
3. ~~**Profile switching buggy**~~ ✅ FIXED + TESTED 2026-07-06 (fix/ux5-profile-switch, UX-5) — stale-post-switch renderer and add-profile reload loop both resolved (full renderer refresh on switch; add-profile routes to onboarding). Still worth one real multi-profile-machine pass before beta; `delete`/`update` remain untested live.
