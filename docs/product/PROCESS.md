...
- **Money**: the loop spends nothing beyond the sanctioned test-money protocol (see "On-chain test-money protocol" below). Default is free-tier: uploads <105 KiB cost 0 credits; on-chain agents abort on any nonzero cost estimate and assert an unchanged balance. Paid (≥105 KiB) uploads are individually capped and balance-checked. Releases/tags/version bumps and any spend outside this protocol still stop for Phil.
